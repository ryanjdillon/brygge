...
<template>
  <div>
    ...
    <template v-else>
      <!-- User's own status cards (if on waiting list) -->
      <template v-if="entry">
      ...
      <!-- Full waiting list table -->
      <div v-if="listEntries && listEntries.length > 0" class="mt-8">
        <h2 class="text-lg font-semibold text-gray-900">{{ t('portal.waitingList.queueOverview') }}</h2>
        <p class="mt-1 text-sm text-gray-500">{{ t('portal.waitingList.queueDescription') }}</p>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('portal.waitingList.name') }}</th>
                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('portal.waitingList.localStatus') }}</th>
                <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('portal.boats.beam') }}</th>
              </tr>
            </thead>
                ...
                <td class="whitespace-nowrap px-4 py-3 text-sm">
                  <span
                    :class="[
                      ...
                      item.is_local ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800',
                    ]"
                  >
                    <MapPin class="h-3 w-3" />
                    {{ item.is_local ? t('portal.waitingList.local') : t('portal.waitingList.nonLocal') }}
                  </span>
                </td>
                <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-500">
                  {{ item.boat_beam ? `${item.boat_beam} m` : '—' }}
                </td>
              </tr>
            </tbody>
          </table>
