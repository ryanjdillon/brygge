...
describe('router guards', () => {
  ...
  it('/admin redirects to / when user lacks admin/board role', async () => {
    const auth = useAuthStore()
    await auth.ready
    auth.user = { id: '1', firstName: 'Test', lastName: '', name: 'Test', email: 'test@example.com', clubId: 'c1', roles: ['member'], totpEnabled: false, totpVerifiedAt: null }
  ...
  it('/admin is accessible with admin role', async () => {
    const auth = useAuthStore()
    await auth.ready
    auth.user = { id: '1', firstName: 'Test', lastName: '', name: 'Test', email: 'test@example.com', clubId: 'c1', roles: ['admin'], totpEnabled: false, totpVerifiedAt: null }
  ...
  it('/admin is accessible with board role', async () => {
    const auth = useAuthStore()
    await auth.ready
    auth.user = { id: '1', firstName: 'Test', lastName: '', name: 'Test', email: 'test@example.com', clubId: 'c1', roles: ['board'], totpEnabled: false, totpVerifiedAt: null }
